...
def get_text_ensemble():
    return VotingClassifier([
        ...
        ('rf', RandomForestClassifier(n_estimators=200, max_depth=5, min_samples_leaf=3, class_weight='balanced', random_state=RANDOM_STATE)),
        ('gb', GradientBoostingClassifier(n_estimators=100, learning_rate=0.05, max_depth=3, subsample=0.8, random_state=RANDOM_STATE))
    ], voting='soft')

def get_audio_ensemble():
    return VotingClassifier([
        ('gb', GradientBoostingClassifier(n_estimators=150, learning_rate=0.05, max_depth=3, subsample=0.8, random_state=RANDOM_STATE)),
        ('rf', RandomForestClassifier(n_estimators=200, max_depth=6, min_samples_leaf=2, class_weight='balanced', random_state=RANDOM_STATE)),
...
def get_visual_ensemble():
    return VotingClassifier([
        ('rf', RandomForestClassifier(n_estimators=150, max_depth=5, min_samples_leaf=3, class_weight='balanced', random_state=RANDOM_STATE)),
...
for t in [threshold, 0.38]:
    preds = (all_prob_fusion >= t).astype(int)
    print('t=', t, confusion_matrix(all_y, preds).tolist())
